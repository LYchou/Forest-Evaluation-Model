...
%default_wage_value=Default_boundary(type_default,step,T,0,tax_firm,c,r,wage);
           ...
           for i=1:Nodes_100(a,j)%不同層樹所展出去的上界皆不同， i=1 為破產邊界
               ...
               for b=a_node:Percentage
                   ...
                   if abs( firm_tree_value_100(i_node,j) - default_boundary_value(j) ) > 1
                       if j==step+1%最後一期
                           %股東價值--------------------------------------------
                           Equity_value(a,b,i_node,j)=max(firm_tree_value_100(i_node,j)-F*(1+(1-tax_firm)*c*t)-O*Div*t-(1-tax_firm)*wage*t+( b - a_node )*X*N*percent,0);
                           %每股價格--------------------------------------------
...
%                            end
                       elseif j>=step*T_exercise/T+1&&j~=1%員工可以履約
                           ...
                           elseif n==0 && V_t>default_boundary_value(j)
                               %在t+delta時點下的最適轉換比率Alpha---------------
                       ...
                       elseif j<step*T_exercise/T+1||j==1%員工不可以履約
                           if b==1 %在歸屬日前都是0%，這是0%的矩陣位置
                               ...
                               elseif n==0 && V_t>default_boundary_value(j)
                                   ...
                                   %ESO價值------------------------------------
                                   TB_value(a,b,i_node,j)=expected_value(Pu,Pm,Pd, TB_value(next_time_a,alpha_u,BTT_B_node-1,j+1) , TB_value(next_time_a,alpha_m,BTT_B_node,j+1) , 0 ,r,t)+tax_firm*F*c*t+tax_firm*wage*t;
                                   BC_value(a,b,i_node,j)=expected_value(Pu,Pm,Pd, BC_value(next_time_a,alpha_u,BTT_B_node-1,j+1) , BC_value(next_time_a,alpha_m,BTT_B_node,j+1) , w*exp(log(default_boundary_value(j+1))-2*sigma*sqrt(t)) ,r,t);
                                   %計算員工年薪(已扣稅)
                                   Wealth=Tax_employee(type_tax_employee,tax_employee,t,wage,additional_wealth,X,X,a_node,b,N,percent,Div,O);
                                   %計算員工效用函數算
               ...
               %判斷是否破產          
                if abs( firm_tree_value_100(i_node,j) - default_boundary_value(j) ) >1 || j==1
                   ...
                   elseif j>=step*T_exercise/T+1&&j~=1
                       %員工最適履約後的資產價值---------------------------------
                       V_t=firm_tree_value_100(i_node,j)-(1-tax_firm)*F*c*t-O*Div*t-(1-tax_firm)*wage*t+( alpha_Utility - a_node )*X*N*percent;
