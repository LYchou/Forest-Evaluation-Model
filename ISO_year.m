...
%計算破產邊界---------------------------------------------------------------
default_boundary_value=Default_boundary(type_default,step,T,F,tax_firm,c,r,wage);%資產樹上界初始值-----------------------------------------------------------
firm_value=V;
           ...
           for i=1:Nodes(a,j)%不同層樹所展出去的上界皆不同， i=1 為破產邊界
               ...
               for b=a_node:Percentage
                   ...
                   if abs( firm_tree_value(i_node,j) - default_boundary_value(j) ) > 10^-10
                       if j==step+1%最後一期
                           %股東價值--------------------------------------------
                           Equity_value(a,b,i_node,j)=max(firm_tree_value(i_node,j)-F*(1+(1-tax_firm)*c*t)-O*Div*t-(1-tax_firm)*wage*t+( b - a_node )*X*N*percent,0);
                           %每股價格--------------------------------------------
                           ...
                           if j>step*T_duration/T+1 %如果超過到期日 都不能履約 excercise=false
                                [Wealth,excercise]=Tax_employee(type_tax_employee,tax_employee,t,wage,additional_wealth,X,X,a_node,b,N,percent,Div,O);
                           end
                           %判斷員工的最適效用是否會履約ESO
...
%                            end
                       elseif j>=step*T_exercise/T+1&&j~=1%員工可以履約
                           ...
                           if n>0
                               ...
                               %計算員工財富(已扣稅)
                               [Wealth,excercise]=Tax_employee(type_tax_employee,tax_employee,t,wage,additional_wealth,Stock_value(a,b,i_node,j),X,a_node,b,N,percent,Div,O);
                               if j>step*T_duration/T+1 %如果超過到期日 都不能履約
                                          [Wealth,excercise]=Tax_employee(type_tax_employee,tax_employee,t,wage,additional_wealth,X,X,a_node,b,N,percent,Div,O);
                               end
                               %判斷員工的最適效用是否會履約ESO
...
% 
%                                %ESO價值-----------------------------------------
%                                TB_value(a,b,i_node,j)=expected_value(Pu,Pm,Pd, TB_value(next_time_a,alpha_u,BTT_B_node-1,j+1) , TB_value(next_time_a,alpha_m,BTT_B_node,j+1) , 0 ,r,t)+F*tax_firm*c*t;
%                                BC_value(a,b,i_node,j)=expected_value(Pu,Pm,Pd, BC_value(next_time_a,alpha_u,BTT_B_node-1,j+1) , BC_value(next_time_a,alpha_m,BTT_B_node,j+1) , w*exp(log(default_boundary_value(j+1)-2*sigma*t)),r,t);
% 
%                                %計算員工財富(已扣稅)
%                                [Wealth,excercise]=Tax_employee(type_tax_employee,tax_employee,t,wage,Stock_value(a,b,i_node,j),X,a_node,b,N,percent,Div,O);
%                                %判斷員工的最適效用是否會履約ESO
%                                [Qu,Qm,Qd]=BTT(sigma,r,mu,t,V_t,default_boundary_value(j+1));
%                                Q=[Q;i_node,j,a_node,b,n,V_t,default_boundary_value(j+1),Qu,Qm,Qd,Qu+Qm+Qd];
...
% 
%                                    %ESO價值------------------------------------
%                                    TB_value(a,b,i_node,j)=expected_value(Pu,Pm,Pd, TB_value(next_time_a,alpha_u,BTT_B_node-1,j+1) , TB_value(next_time_a,alpha_m,BTT_B_node,j+1) , 0 ,r,t);
%                                    BC_value(a,b,i_node,j)=expected_value(Pu,Pm,Pd, BC_value(next_time_a,alpha_u,BTT_B_node-1,j+1) , BC_value(next_time_a,alpha_m,BTT_B_node,j+1) , w*exp(log(default_boundary_value(j+1)-2*sigma*t)) ,r,t);
%                                    %計算員工年薪(已扣稅)
%                                    Wealth=Tax_employee(type_tax_employee,tax_employee,t,wage,X,X,a_node,b,N,percent,Div,O);
%                                    %計算員工效用函數算
%                                    [Qu,Qm,Qd]=BTT(sigma,r,mu,t,V_t,default_boundary_value(j+1));
%                                    Q=[Q;i_node,j,a_node,b,n,V_t,default_boundary_value(j+1),Qu,Qm,Qd,Qu+Qm+Qd];
%                                    Utility_value(a,b,i_node,j)=Wealth^gamma + expected_value(Qu,Qm,Qd, Utility_value(next_time_a,alpha_u,BTT_B_node-1,j+1) , Utility_value(next_time_a,alpha_m,BTT_B_node,j+1) , 0 ,Beta,t);
               ...
               %判斷是否破產          
                if abs( firm_tree_value(i_node,j) - default_boundary_value(j) ) >10^-10 || j==1
                   ...
                   elseif j>=step*T_exercise/T+1&&j~=1
                       %員工最適履約後的資產價值---------------------------------
                       V_t=firm_tree_value(i_node,j)-(1-tax_firm)*F*c*t-O*Div*t-(1-tax_firm)*wage*t+( alpha_Utility - a_node )*X*N*percent;
                        if j>step*T_duration/T+1
                           V_t=firm_tree_value(i_node,j)-(1-tax_firm)*F*c*t-O*Div*t-(1-tax_firm)*wage*t;
                       end
                       ...
                       if n>0
                           ...
                           if (Payoff==0 && alpha_Utility>a_node) || j>step*T_duration/T+1
