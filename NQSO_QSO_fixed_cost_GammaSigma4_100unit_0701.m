...
clear
% 設定說明
A_discription = '延續NQSO_QSO_fixed_cost_GammaSigma3_100unit.m的程式，改成4，percent=0.1,step_unit=1,threshold=100,分工合作 我跑sigma小的case';
% 執行日期 (用於存圖的檔名)
run_date = '20210701';
%固定變數------------------------
V=9.91e9;
...
for list_1=1:length(gamma)
    for list_2=1:length(sigma)
        ...
        while 1
           ...
           % 二分法搜尋
           [wage,ESO,Salary,P_check,Q_check,minError_parameter] = binary_search_fixed_cost4(ESO_type,fixed_cost,searchTimes,threshold,previousWage_right,V,sigma(list_2),T,T_exercise,step,F,c,r,mu,X,N,O,additional_wealth,gamma(list_1),tax_employee,Beta,w,tax_firm,percent,type_default,method,Div);
