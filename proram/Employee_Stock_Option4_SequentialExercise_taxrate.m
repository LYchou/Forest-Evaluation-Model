function [Equity,Debt,Stock,ESO,Utility,alpha,P,Q,ESO_value,Stock_value,Equity_value,Debt_value,Utility_value,Dividend_value,Dividend,TB_value,BC_value,TB,BC,Salary_value,Salary,max_time_node_100,Nodes_100,firm_tree_value_100,default_boundary_value]=Employee_Stock_Option4_SequentialExercise_taxrate(V,sigma,T,T_exercise,step,F,c,r,mu,X,N,O,wage,additional_wealth,gamma,tax_employee,Beta,w,tax_firm,percent,type_default,type_tax_employee,method,Div)
% 更改
% 延續Employee_Stock_Option3改過 default的判斷式，加入名勛說的:增加選點上界，所以上界中點硬修正的部分就註解掉
% 還有default_wage_value全部設成零
% 增加紀錄履約稅率的矩陣
%
%
%sigma=公司資產價值標準差(%)
...
% 2021/06/11 在計算ESO成本地方(可履約的時間區間)也加上中點的硬修正
...
default_wage_value=zeros(size(default_boundary_value)); 
%資產樹上界初始值-----------------------------------------------------------
% firm_value=V;
...
% end
%0611新增：浩瀚資產樹100%履約上屆
firm_value_100=V;
Up_boundary_value_100=[];Nodes_100=[];
for i=Percentage:-1:1 %找出各層資產數與各個時期的上界
    [up_boundary_value_100,nodes_100]=Up_boundary100(step,r,sigma,T,T_exercise,firm_value_100,default_boundary_value,X,N,percent); %上界
    Up_boundary_value_100=[up_boundary_value_100;Up_boundary_value_100]; %上界離DB幾個點(2sigma根號delta t)
    Nodes_100(i,:)=nodes_100;
...
[firm_tree_value_100,max_time_node_100]=Firm_tree_value(V,sigma,step,t,default_boundary_value,Nodes_100);
%建立個價值樹---------------------------------------------------------------
Equity_value=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
Stock_value=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
Debt_value=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
ESO_value=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
Utility_value=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
alpha=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
Dividend_value=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
TB_value=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
BC_value=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
Salary_value=zeros(Percentage,Percentage,max(max_time_node_100),step+1);
P=[];Q=[];

TaxRate = zeros(Percentage,Percentage,max(max_time_node_100),step+1);
...
%       for j=step*T_exercise/T:-1:2
       for a=1:Percentage%目前所在的資產樹是以贖回多少百分比， a=1 為 100% ; a=Percentage 為 0%
           ...
           for i=1:Nodes_100(a,j)%不同層樹所展出去的上界皆不同， i=1 為破產邊界
               %在j時點時的資產價值高度，i=1 為破產邊界，往上建
               i_node=max(max_time_node_100)+1-i;
               for b=a_node:Percentage %0% 50% 100%
                   %已知員工下一期贖回a%的數值轉換=這一期的b%
                   next_time_a=Percentage-b+1;
                   %判斷是否破產
                   if abs( firm_tree_value_100(i_node,j) - default_boundary_value(j) ) > 1
                       if j==step+1%最後一期
                           %股東價值--------------------------------------------
                           %ESO稅盾加入N*(S-X)*tax
                           Equity_value(a,b,i_node,j)=max((firm_tree_value_100(i_node,j)-F*(1+(1-tax_firm)*c*t)-O*Div*t-(1-tax_firm)*wage*t+(1-tax_firm)*( b - a_node )*X*N*percent)/(1-tax_firm*(( b - a_node )*N*percent/(O+(b-1)*N*percent))),0);
                           %每股價格--------------------------------------------
                           ...
                           %計算員工財富(已扣稅)
                           [Wealth,excercise,max_tax_rate]=Tax_employee2(type_tax_employee,tax_employee,t,wage,additional_wealth,Stock_value(a,b,i_node,j),X,a_node,b,N,percent,Div,O);
                           TaxRate(a,b,i_node,j) = max_tax_rate;
                           %判斷員工的最適效用是否會履約ESO
                       ...
                       elseif j>=step*T_exercise/T+1&&j~=1%員工可以履約
                           %員工履約後的資產價值--------------------------------
                           V_t=firm_tree_value_100(i_node,j)-(1-tax_firm)*F*c*t-O*Div*t-(1-tax_firm)*wage*t+( b - a_node )*X*N*percent;
                           %BTT計算機率-----------------------------------------
...
%                                        end
    ...
    %                                    P=[P;i_node,j,a_node,b,n,V_t,default_boundary_value(j+1),Pu,Pm,Pd,Pu+Pm+Pd];
                               ...
                               %債券價值----------------------------------------
                               Debt_value(a,b,i_node,j)=(1-w)*(firm_tree_value_100(i_node,j)-default_wage_value(j));
                               Dividend_value(a,b,i_node,j)=0;
                               %ESO價值-----------------------------------------
                               TB_value(a,b,i_node,j)=0;
                               BC_value(a,b,i_node,j)=w*(firm_tree_value_100(i_node,j)-default_wage_value(j));
                       ...
                       elseif j<step*T_exercise/T+1||j==1%員工不可以履約
                           if b==1 %在歸屬日前都是0%，這是0%的矩陣位置
                               %員工履約後的資產價值-----------------------------
                               if j>1
                                   V_t=firm_tree_value_100(i_node,j)-(1-tax_firm)*F*c*t-O*Div*t-(1-tax_firm)*wage*t;
                               else
                                   V_t=V;
                               end
                               %BTT計算機率-------------------------------------
                               ...
                               if n>0 
                                   ...
                                   %ESO價值-------------------------------------
                                   %計算員工年薪(已扣稅)
                                   [Wealth,~,max_tax_rate]=Tax_employee2(type_tax_employee,tax_employee,t,wage,additional_wealth,X,X,a_node,b,0,percent,Div,O);
                                   TaxRate(a,b,i_node,j) = max_tax_rate;
                                   %計算員工效用函數算
                                   [Qu,Qm,Qd]=BTT(sigma,r,mu,t,V_t,default_boundary_value(j+1));
                                   Q=[Q;i_node,j,a_node,b,n,V_t,default_boundary_value(j+1),Qu,Qm,Qd,Qu+Qm+Qd];
                               ...
                               elseif n==0 && V_t>default_boundary_value(j) %已知中間點接到下期的違約門檻
                                   ...
                                   %計算員工年薪(已扣稅)
                                   [Wealth,~,max_tax_rate]=Tax_employee2(type_tax_employee,tax_employee,t,wage,additional_wealth,X,X,a_node,b,N,percent,Div,O);
                                   TaxRate(a,b,i_node,j) = max_tax_rate;
                                   %計算員工效用函數算
                                   [Qu,Qm,Qd]=BTT(sigma,r,mu,t,V_t,default_boundary_value(j+1));
                                   Q=[Q;i_node,j,a_node,b,n,V_t,default_boundary_value(j+1),Qu,Qm,Qd,Qu+Qm+Qd];
                                   ...
                                   Equity_value(a,b,i_node,j)=0;
                                   %債券價值------------------------------------
                                   Debt_value(a,b,i_node,j)=(1-w)*(firm_tree_value_100(i_node,j)-default_wage_value(j));%清算後的錢
                                   Dividend_value(a,b,i_node,j)=0;
                                   %ESO價值-----------------------------------------
                                   TB_value(a,b,i_node,j)=0;
                                   BC_value(a,b,i_node,j)=w*(firm_tree_value_100(i_node,j)-default_wage_value(j)); %剩下的錢的清算價值
               ...
               %判斷是否破產                           
               if abs( firm_tree_value_100(i_node,j) - default_boundary_value(j) ) >1 || j==1
                   if j==step+1
                       ESO_value(a,alpha_Utility,i_node,j)=N*( alpha_Utility - a_node )*percent*Payoff;
                   elseif j>=step*T_exercise/T+1&&j~=1
                       %員工最適履約後的資產價值---------------------------------
                       V_t=firm_tree_value_100(i_node,j)-(1-tax_firm)*F*c*t-O*Div*t-(1-tax_firm)*wage*t+( alpha_Utility - a_node )*X*N*percent+tax_firm*( alpha_Utility - a_node )*Payoff*N*percent;
...
%                                        end
                           ...
                           elseif n==0
                               ...
                               %ESO價值-----------------------------------------
                               TB_value(a,b,i_node,j)=0;
                               BC_value(a,b,i_node,j)=w*firm_tree_value_100(i_node,j);
...
%                                V_t=firm_tree_value_100(i_node,j)-(1-tax_firm)*F*c*t-O*Div*t-(1-tax_firm)*wage*t;
    ...
    BTT_B_node=max(max_time_node_100)-n;
    P=[P;max(max_time_node_100),1,BTT_B_node,1,n,V,default_boundary_value(2),Pu,Pm,Pd,Pu+Pm+Pd];
    %在t t時點下的最適轉換比率Alpha---------
    if T_exercise==0
...
SIZE=size(Debt_value);
sizei=SIZE(3);
sizet=SIZE(4);
maxper_layer=1+1/percent;
for idx=1:sizei
    for idxx=sizet:-1:1
            result1(idx,idxx)=(alpha(maxper_layer,1,idx,idxx)); %+Debt_value(1,1,idx,idxx)+ESO_value(1,1,idx,idxx)-TB_value(1,1,idx,idxx)+BC_value(1,1,idx,idxx)+Salary_value(1,1,idx,idxx);
            result2(idx,idxx)=(alpha(maxper_layer,2,idx,idxx)); %+Debt_value(1,2,idx,idxx)+ESO_value(1,2,idx,idxx)-TB_value(1,2,idx,idxx)+BC_value(1,2,idx,idxx)+Salary_value(1,2,idx,idxx);
            result3(idx,idxx)=(alpha(maxper_layer,3,idx,idxx)); %+Debt_value(1,3,idx,idxx)+ESO_value(1,3,idx,idxx)-TB_value(1,3,idx,idxx)+BC_value(1,3,idx,idxx)+Salary_value(1,3,idx,idxx);
            result4(idx,idxx)=(alpha(maxper_layer,4,idx,idxx)); %+Debt_value(2,1,idx,idxx)+ESO_value(2,1,idx,idxx)-TB_value(2,1,idx,idxx)+BC_value(2,1,idx,idxx)+Salary_value(2,1,idx,idxx);
            result5(idx,idxx)=(alpha(maxper_layer,5,idx,idxx)); %+Debt_value(2,2,idx,idxx)+ESO_value(2,2,idx,idxx)-TB_value(2,2,idx,idxx)+BC_value(2,2,idx,idxx)+Salary_value(2,2,idx,idxx);
            result6(idx,idxx)=(alpha(maxper_layer,6,idx,idxx)); %+Debt_value(2,3,idx,idxx)+ESO_value(2,3,idx,idxx)-TB_value(2,3,idx,idxx)+BC_value(2,3,idx,idxx)+Salary_value(2,3,idx,idxx);
            result7(idx,idxx)=(alpha(maxper_layer,7,idx,idxx)); %+Debt_value(3,1,idx,idxx)+ESO_value(3,1,idx,idxx)-TB_value(3,1,idx,idxx)+BC_value(3,1,idx,idxx)+Salary_value(3,1,idx,idxx);
            result8(idx,idxx)=(alpha(maxper_layer,8,idx,idxx)); %+Debt_value(3,1,idx,idxx)+ESO_value(3,2,idx,idxx)-TB_value(3,2,idx,idxx)+BC_value(3,2,idx,idxx)+Salary_value(3,2,idx,idxx);
            result9(idx,idxx)=(alpha(maxper_layer,9,idx,idxx)); %+Debt_value(3,1,idx,idxx)+ESO_value(3,3,idx,idxx)-TB_value(3,3,idx,idxx)+BC_value(3,3,idx,idxx)+Salary_value(3,3,idx,idxx);
            result10(idx,idxx)=(alpha(maxper_layer,10,idx,idxx)); %+Debt_value(3,1,idx,idxx)+ESO_value(3,3,idx,idxx)-TB_value(3,3,idx,idxx)+BC_value(3,3,idx,idxx)+Salary_value(3,3,idx,idxx);
            result11(idx,idxx)=(alpha(maxper_layer,11,idx,idxx)); %+Debt_value(3,1,idx,idxx)+ESO_value(3,3,idx,idxx)-TB_value(3,3,idx,idxx)+BC_value(3,3,idx,idxx)+Salary_value(3,3,idx,idxx);
            resultstock(idx,idxx)=(Stock_value(maxper_layer,1,idx,idxx));
            
            
            maxper_layer_node=Percentage-maxper_layer+1;
            % tax rate
            %result_taxrate(idx,idxx)=TaxRate(maxper_layer,1,idx,idxx); 
            alpha_Utility = (alpha(maxper_layer,1,idx,idxx)/percent)+1;
            result1_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result1_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,2,idx,idxx)/percent)+1;
            result2_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result2_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,3,idx,idxx)/percent)+1;
            result3_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result3_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,4,idx,idxx)/percent)+1;
            result4_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result4_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,5,idx,idxx)/percent)+1;
            result5_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result5_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,6,idx,idxx)/percent)+1;
            result6_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result6_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,7,idx,idxx)/percent)+1;
            result7_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result7_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,8,idx,idxx)/percent)+1;
            result8_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result8_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,9,idx,idxx)/percent)+1;
            result9_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result9_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,10,idx,idxx)/percent)+1;
            result10_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result10_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            
            alpha_Utility = (alpha(maxper_layer,11,idx,idxx)/percent)+1;
            result11_taxrate(idx,idxx) = TaxRate(maxper_layer,alpha_Utility,idx,idxx); 
            result11_payoff(idx,idxx) = (Stock_value(maxper_layer,alpha_Utility,idx,idxx) - X)*(alpha_Utility - maxper_layer_node);
            %resultstock(idx,idxx)=(Stock_value(maxper_layer,1,idx,idxx));
    end
end
resultall=zeros(size(result1));
resultall_taxrate=zeros(size(result1));
resultall=result1+result2+result3+result4+result5+result6+result7+result8+result9+result10+result11;

result1_taxrate_adj = compare2matrix_for_SequentialExer(result1,result1_taxrate);
result2_taxrate_adj = compare2matrix_for_SequentialExer(result2,result2_taxrate);
result3_taxrate_adj = compare2matrix_for_SequentialExer(result3,result3_taxrate);
result4_taxrate_adj = compare2matrix_for_SequentialExer(result4,result4_taxrate);
result5_taxrate_adj = compare2matrix_for_SequentialExer(result5,result5_taxrate);
result6_taxrate_adj = compare2matrix_for_SequentialExer(result6,result6_taxrate);
result7_taxrate_adj = compare2matrix_for_SequentialExer(result7,result7_taxrate);
result8_taxrate_adj = compare2matrix_for_SequentialExer(result8,result8_taxrate);
result9_taxrate_adj = compare2matrix_for_SequentialExer(result9,result9_taxrate);
result10_taxrate_adj = compare2matrix_for_SequentialExer(result10,result10_taxrate);
result11_taxrate_adj = compare2matrix_for_SequentialExer(result11,result11_taxrate);

resultall_taxrate=result1_taxrate_adj+result2_taxrate_adj+result3_taxrate_adj+result4_taxrate_adj+result5_taxrate_adj+result6_taxrate_adj+result7_taxrate_adj+result8_taxrate_adj+result9_taxrate_adj+result10_taxrate_adj+result11_taxrate_adj;

result1_payoff_adj = compare2matrix_for_SequentialExer(result1,result1_payoff);
result2_payoff_adj = compare2matrix_for_SequentialExer(result2,result2_payoff);
result3_payoff_adj = compare2matrix_for_SequentialExer(result3,result3_payoff);
result4_payoff_adj = compare2matrix_for_SequentialExer(result4,result4_payoff);
result5_payoff_adj = compare2matrix_for_SequentialExer(result5,result5_payoff);
result6_payoff_adj = compare2matrix_for_SequentialExer(result6,result6_payoff);
result7_payoff_adj = compare2matrix_for_SequentialExer(result7,result7_payoff);
result8_payoff_adj = compare2matrix_for_SequentialExer(result8,result8_payoff);
result9_payoff_adj = compare2matrix_for_SequentialExer(result9,result9_payoff);
result10_payoff_adj = compare2matrix_for_SequentialExer(result10,result10_payoff);
result11_payoff_adj = compare2matrix_for_SequentialExer(result11,result11_payoff);

resultall_payoff = result1_payoff_adj+result2_payoff_adj+result3_payoff_adj+result4_payoff_adj+result5_payoff_adj+result6_payoff_adj+result7_payoff_adj+result8_payoff_adj+result9_payoff_adj+result10_payoff_adj+result11_payoff_adj;
