...
clear
% 設定說明
A_discription = '使用debug的程式 3，固定成本下，討論不同gamma不同sigma下三種最佳化的結果，maxN=100，將股票報酬率與波動度校正成資產，同時調整Beta。';
% 執行日期 (用於存圖的檔名)
run_date = '2021....';
%固定變數------------------------
V=9.91e9;
...
for list_1=1:length(gamma)
    for list_2=1:length(sigma)
        ...
        while 1
           ...
           % 二分法搜尋
           [wage,ESO,Salary,minError_parameter] = binary_search_fixed_cost3(ESO_type,fixed_cost,searchTimes,threshold,previousWage_right,V,sigma(list_2),T,T_exercise,step,F,c,r,mu,X,N,O,additional_wealth,gamma(list_1),tax_employee,Beta,w,tax_firm,percent,type_default,method,Div);
